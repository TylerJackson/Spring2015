...
int main(int argc, char **argv)
{
  ...
  //string secret = "80533897"; //aloha with ever 3rd bit removed
                              //and put in ip-id
                              //in text to hex to decimal
  ...
  iph->ip_v = 4;
  iph->ip_tos = 0;
  iph->ip_len = sizeof (struct ip) + sizeof (struct tcphdr) + 8 +6 +6 + sizeof(datagram); /* data size = 0, but tcp using option flags */
  iph->ip_id = htons (2414);
  iph->ip_off = 0;
  iph->ip_ttl = 250;
  iph->ip_p = 6;
  iph->ip_sum = 0;
  //iph->ip_sum = (uint16_t)19369;
  ...
  //tcph->th_seq = htonl("414c4f4841");
  tcph->th_seq = htonl(random());
